...
FORMULAE: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "symbols": {
            ...
            "T_allow": ("available time minus pipeline latency", "ns", "SystemConfig.DPU_ACLK_MHz"),
            "PTW": ("page-table-walk ratio", "ratio", "SystemConfig.PTW"),
        },
...
def solve(config: SimulatorConfig) -> Result:
    ...
    rotation = _rotation_preload(config, per_layer, time, dsc, flags)
...
def _rotation_preload(
    ...
    for layer in config.layers:
        info = _format_info(layer.fmt)
        if layer.compressed:
            compression_cycles += layer.src_h * info.lmc / system.ppc_comp
        if layer.scaling:
            scaler_cycles += layer.src_h * system.vtap_scaler / system.ppc_scaler
        if layer.rotation:
    ...
    base = {
        ...
        "status": "N/A (DPU_ACLK_MHz required)",
    }

    if rotation_data == 0:
        base["status"] = "N/A (no rotation layers)"
        return base
    if system.DPU_ACLK_MHz is None:
        flags["F3"] = "DPU_ACLK_MHz is required for IB_rotation_preload."
        return base
    if system.DPU_ACLK_MHz <= 0:
        raise ValueError("DPU_ACLK_MHz must be positive when provided")

    latency_time = pipeline_cycles * 1000 / system.DPU_ACLK_MHz
...
def breakdown_rows(result: Result) -> list[dict[str, str]]:
    ...
    for key, value in result.rotation.items():
        extra = "F3/F5" if key != "status" else str(value)
...
def term_summary_rows(result: Result) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
...
def build_formula_html(result: Result) -> str:
    ...
    for formula in FORMULAE:
        ...
        parts.append("</table>")
        if key in {"IB_streaming", "IB_rotation_preload"}:
            flag_ids = ["F1", "F2"] if key == "IB_streaming" else ["F3", "F5"]
...
def _formula_value_table(formula: dict[str, Any], result: Result) -> str:
    ...
    term_key = f"{formula['key']}_MBps"
    rows.append(
        "<tr>"
        f"<td>{html.escape(formula['key'])}</td>"
        f"<td>{html.escape(_fmt(result.terms.get(term_key)))}</td>"
        "<td>MB/s</td>"
        "</tr>"
    )
    rows.append("</table>")
    ...
    class SimulatorWindow(QMainWindow):
        ...
        def add_layer(self) -> None:
            self.config.layers.append(
                LayerConfig("Layer", 1080, 1080, "RGB_4B", False, False, 1.0, False, 0, 1.0)
            )
            self._populate_inputs()
            self.recalculate()
        ...
        def read_config(self) -> SimulatorConfig:
            ...
            aclk_text = str(system_values["DPU_ACLK_MHz"]).strip()
            system = SystemConfig(
                DPU_ACLK_MHz=None if not aclk_text else float(aclk_text),
            ...
            for row in range(self.layers_table.rowCount()):
                ...
                for col, name in enumerate(self.layer_columns):
                    ...
                    if name in {"compressed", "scaling", "rotation"}:
        ...
        def _refresh_summary(self, result: Result) -> None:
            ...
            for term in term_summary_rows(result):
                value_text = (
        ...
        def _refresh_breakdown(self, result: Result) -> None:
            rows = breakdown_rows(result)

            self.breakdown.setRowCount(len(rows))
            self.breakdown.setColumnCount(5)
            self.breakdown.setHorizontalHeaderLabels(["group", "name", "model", "measured", "note"])
            ...
            if not getattr(self, "_breakdown_columns_initialized", False):
                for col, width in enumerate([90, 210, 135, 110, 260]):
